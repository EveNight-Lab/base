...
export class ApiService {
  ...
  async request<T>(url: string, options: ApiRequestOptions = {}): Promise<T> {
    ...
    // 요청 식별 및 flowTracker 로깅 시작
    const requestId = `api-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    let requestBody: unknown = undefined;
    if (fetchOptions.body && typeof fetchOptions.body === 'string') {
      try {
        requestBody = JSON.parse(fetchOptions.body) as unknown;
      } catch {
        requestBody = fetchOptions.body;
      }
    }

    // Lazy import flowTracker to avoid circular dependency
    const { flowTracker } = await import('./flowTracker');

    flowTracker.logApiStart(requestId, method, url, requestBody);
    ...
    if (callerSignal) {
      if (callerSignal.aborted) {
        ...
        flowTracker.logApiAbort(requestId, duration);
    ...
    try {
      // 1. 네트워크 지연 시뮬레이션
      if (flowTracker.simulateLatency) {
        await new Promise<void>((resolve, reject) => {
          const delayTimer = setTimeout(resolve, 1500);
      ...
      // 2. 네트워크 에러 시뮬레이션
      if (flowTracker.simulateError) {
        throw new ApiError(
          500,
          'Simulated Error',
          '시뮬레이션: Dev Console에서 네트워크 에러 시뮬레이션이 켜져 있습니다.'
        );
      }
      ...
      // 성공 로그 기록
      flowTracker.logApiSuccess(requestId, response.status, responseData, duration);
      ...
      if (error instanceof ApiError) {
        flowTracker.logApiFailure(requestId, error.message, duration, error.status);
        throw error;
      }

      const errMsg = error instanceof Error ? error.message : 'Unknown error';
      flowTracker.logApiFailure(requestId, `Network error: ${errMsg}`, duration, 500);
